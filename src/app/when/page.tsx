...
import { findRecentQualifyingEvents } from "@/lib/eventAggregation";
...
export default async function WhenPage({
  ...
  const branch = paramStr(params.branch) === "event" ? "event" : "fight";
    ...
    <div className="flex flex-col flex-1 items-center bg-zinc-50 font-sans dark:bg-black">
      <main className="flex w-full max-w-3xl flex-1 flex-col gap-6 px-6 py-12">
        ...
        <nav className="flex gap-4 border-b border-zinc-200 pb-4 text-sm dark:border-zinc-800">
          <Link
            href={{ pathname: "/when", query: { branch: "fight" } }}
            className={
              branch === "fight"
          ...
          <Link
            href={{ pathname: "/when", query: { branch: "event" } }}
            className={
              branch === "event"
                ? "font-semibold text-zinc-900 dark:text-zinc-50"
                : "text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-50"
        ...
        {branch === "fight" ? (
          <FightBranch params={params} />
        ) : (
          <EventBranch params={params} />
        )}
      </main>
    </div>
  );
...
async function FightBranch({
  ...
  const carryForward = { term, division };
